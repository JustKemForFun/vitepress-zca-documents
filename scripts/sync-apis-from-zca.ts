...
const DOCS_APIS_DIR = (lang: "vi" | "en") => path.join(ROOT, "docs", "pages", lang, "apis");
const SNIPPETS_API_DIR = (apiName: string) => path.join(ROOT, "docs", "snippets", "apis", apiName);
...
function parseApiNamesFromApisDts(dts: string): string[] {
...
function exportedTypeNamesFromApiDts(dts: string): string[] {
    return [...dts.matchAll(/export type ([A-Za-z_$][\w$]*)\s*=/g)].map((m) => m[1]);
}

function makeTypesSnippet(apiName: string, typeNames: string[]): string {
...
function makeExampleSnippet(apiName: string): string {
...
function makeApiMarkdown(lang: "vi" | "en", apiName: string, signature: string): string {
    const isVi = lang === "vi";
    const exampleIntro =
...
function inferSignatureFromApiDts(apiName: string, dts: string): string {
...
function main() {
    ...
    const missingByLang: Record<"vi" | "en", string[]> = { vi: [], en: [] };
